      ...
      if ($license = $dao->getActiveLicenseForUser($user->id)) {
        ...
        if (count($aInvoicesDetail)) {
          ...
          if ($license->wksCharged < $license->workstations) {
            ...
            if (2 == $license->workstations) {
              $product = $products->getByName('WKSSTATION1');
            } elseif (3 == $license->workstations) {
              $product = $products->getByName('WKSSTATION2');
            } elseif (4 == $license->workstations) {
              $product = $products->getByName('WKSSTATION3');
            } elseif (5 == $license->workstations) {
              $product = $products->getByName('WKSSTATION4');
            } elseif (6 == $license->workstations) {
              $product = $products->getByName('WKSSTATION5');
            }
          ...
          \sys::logger(sprintf('could not create autocreate invoice : FailedToCreateInvoice : %s', $user->name));
        }
      } else {
        \sys::logger(sprintf('could not create autocreate invoice : NoActiveLicense : %s', $user->name));
      }
    } else {
      \sys::logger('could not create autocreate invoice : InvalidAccount');
    }
  }
    ...
    if ($res = $this->Result($sql)) {
      ...
      $data = $res->dtoSet(function ($dto) use ($debug) {
        ...
        if (!$dto->business_name) {
          $daoGuid = new guid;
          if ($dtoGuid = $daoGuid->getForUser($dto->id)) {
            if ($dtoSite = $daoGuid->getLatestSite($dtoGuid[0]->guid)) {
              if ($dtoSite->site) {
                $this->UpdateByID([
                  'business_name' => $a['business_name'] = $dtoSite->site
                ], $dto->id);
              }
              // \sys::dump($dtoSite);
            }
        ...
        // if (114 == $dto->id) \sys::dump($a);

        $dao = new invoices;
        if ($license = $dao->getActiveLicenseForUser($dto->id)) {
          // if (114 == $dto->id) \sys::dump($license);
