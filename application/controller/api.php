  ...
  protected function checkin($action) {
    ...
    if ($site != '') {
      ...
      if ($a['deployment'] != '') {
        $res = $this->dbResult(
          sprintf(
            "SELECT * FROM SITES WHERE site = '%s' AND workstation = '%s'",
              ...
              if ($uDto = $guidDAO->getUser($a['guid'])) {
                ...
                if (!($dtoUP = $iDAO->getUnpaidForUser($uDto->id))) {
                  ...
                  if ($invID = $uDAO->autoCreateInvoiceFromLast($uDto->id)) {
                    ...
                    if ($uDto->suppress_invoice) {

                      \sys::logger(sprintf('invoice is suppressed for #%s', $uDto->id));
                    } else {
                    ...
                    \sys::logger(sprintf('could not create an invoice ..(%s)', $interval->days));
                  }
                }
              }
            }
        ...
        \sys::logger(sprintf(
          'site: %s, %s, %s, %s @ (%s), %s (activated:%s - %s)',
          $a['site'],
          $a['workstation'],
          $a['version'],
          $a['deployment'],
    ...
    if ($guid = $this->getPost('guid')) {
      ...
      if ($guidDTO = $guidDAO->getByGUID($guid)) {
        ...
        if ((int)$guidDTO->user_id < 1) {
          ...
          \Json::nak(sprintf('%s : account already assigned', $action));
        }
      } else {
        \Json::nak($action);
      }
